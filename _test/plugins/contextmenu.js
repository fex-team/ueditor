...
//        equal(editor.body.getElementsByTagName('table')[0].interlaced,'enabled','表格隔行变色');
        ...
        {
            if(i%2 == 0){
                equal(trs[i].className,'back1','第'+i+'行：浅色行');
            }else{
        ...
        setTimeout(function (){
            lang = editor.getLang( "contextMenu" );
            equal(menutableBody.childNodes.length,2,'2个子项目');
            if(browser.gecko){
                equal(menutableBody.textContent,'表格隔行变色取消表格隔行变色','检查menu显示的字符');
            }
            else{
                equal(menutableBody.innerText.replace( /[\r\n\t\u200b\ufeff]/g, '' ),'表格隔行变色取消表格隔行变色','检查menu显示的字符');
            }
    ...
    setTimeout(function(){
        ...
        setTimeout(function (){
            ...
            setTimeout(function(){
                ...
                tds = editor.body.getElementsByTagName('td');
                equal(ua.getChildHTML(tds[2].firstChild.firstChild),html,'插入了代码');
                /*试图模拟清空文档*/
//                range.setStart( tds[0],0).collapse( true ).select();
//                ua.contextmenu(editor.body.firstChild);
//                var menutableBody = document.getElementsByClassName("edui-menu-body")[0];
...
//                if ( UE.browser.ie ) {
//                    equal( ua.getChildHTML( editor.body ), '<p></p>' );
//                }else {
//                    equal( ua.getChildHTML( editor.body ), '<p><br></p>', '清空文档' );
//                }
                var menuDiv = document.getElementById("edui_fixedlayer");
                menuDiv.parentNode.removeChild(menuDiv);
                div.parentNode.removeChild(div);
