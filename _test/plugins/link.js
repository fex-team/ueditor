...
test( '选中多个单元格插入超链接', function () {
        ...
        range.setStart( tds[0], 0 ).setEnd( tds[2], 0 ).select();
        editor.currentSelectedArr = [tds[0], tds[1], tds[2]];
        editor.execCommand( 'link', {href:'http://www.baidu.com/'} );
        equal( ua.getChildHTML( tds[0] ), '<a href="http://www.baidu.com/">http://www.baidu.com/</a>', '第一个单元格中插入超链接' );
        equal( ua.getChildHTML( tds[1] ), '<a href="http://www.baidu.com/">http://www.baidu.com/</a>', '第二个单元格中插入超链接' );
        equal( ua.getChildHTML( tds[2] ), '<a href="http://www.baidu.com/">hello</a>', '第三个单元格中插入超链接' );
} );
...
test( '去除表格中的链接', function () {
        ...
        var tds = editor.body.firstChild.getElementsByTagName( 'td' );
        range.setStart( tds[0], 0 ).setEnd( tds[1], 0 ).select();
        editor.currentSelectedArr = [tds[0], tds[1], tds[2]];
        editor.execCommand( 'link', {href:'http://www.baidu.com/'} );
        equal( editor.queryCommandValue( 'link' ), tds[0].firstChild, '查询多个单元格的command value为a' );
        editor.execCommand( 'unlink' );
        equal( ua.getChildHTML( tds[0] ), 'http://www.baidu.com/', '第一个单元格中插入超链接' );
        equal( ua.getChildHTML( tds[1] ), 'http://www.baidu.com/', '第二个单元格中插入超链接' );
        equal( ua.getChildHTML( tds[2] ), 'hello', '第三个单元格中插入超链接' );
...
test( '添加链接--表格第一个单元格没有超链接', function () {
        ...
        range = editor.selection.getRange();
        equal( ua.getChildHTML( tds[1] ), '<a href="www.baidu.com">google</a>', 'a标签的地址被修改了' );
        equal( ua.getChildHTML( tds[0] ), '<a href="www.baidu.com">www.baidu.com</a>', 'td 1 被添加了超链接' );
        if ( !baidu.editor.browser.gecko )
                ua.checkResult( range, tds[0].firstChild.firstChild, tds[0].firstChild.firstChild, 0, 13, false, 'check link result' );
        else
                ua.checkResult( range, tds[0].firstChild, tds[0].firstChild, 0, 1, false, 'check link result' );
} );
