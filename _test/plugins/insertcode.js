...
test('trace 3396：多次切换源码，不会产生空行', function () {
    ...
    ua.keydown(editor.body, {'keyCode': 65, 'ctrlKey': true});
    editor.execCommand('insertcode', 'html');
    var br = ua.browser.ie >8 ?  '\n' : '<br>';
    ua.checkSameHtml(editor.body.firstChild.outerHTML.toLowerCase(),  '<pre class="brush:html;toolbar:false">&lt;body&gt;' + br + '&lt;/body&gt;</pre>', '检查插入了html');
    setTimeout(function () {
        editor.execCommand('source');
        setTimeout(function () {
            editor.execCommand('source');
...
test('test-beforeInsertHTML', function () {
    ...
    //插入br element
    editor.setContent('<pre class="brush:html;toolbar:false">Itext</pre>');
