...
13. 修复chrome 34版本下报错问题
14. 修复ie8下插入多张远程图片之后，对话框假死问题
15. 修复uparse,在页面中多次调用某些模块无效果问题
16. 修复容器宽度100%时,改变窗口大小,宽度不自适应的问题

###重构前后端交互功能
1. 多个上传模块使用的上传插件更换成webuploader
2. 改进配置方式，前后端共用的配置项统一在后端配置
3. 改进前后端通信方式，提交请求发送给唯一的地址，后端通过action参数区分请求类型
4. 支持添加自定义请求参数，向后端发送请求时，自动加上参数
5. 快捷上传图片和拖放站贴图片功能，添加loading和上传错误的提示
6. 添加插入在线附件功能，支持滚动拉取图片列表
7. 添加简单上传插件，使用iframe提交上传
