...
//    }
    ...
    editorui.forecolor = function (editor, list, title) {
        title = editor.options.labelMap['forecolor'] || editor.getLang("labelMap.forecolor") || '';
        list = editor.options['forecolor'] || [];
        if (utils.isEmptyObject(list)) return;
        var items = [];
        for (var i in list) {
            items.push({
                value:i,
                label:list[i] || editor.getLang("forecolor")[i],
                theme:editor.options.theme,
                renderLabelHtml:function () {
                    return '<div class="edui-label %%-label"><span class="edui-for-' + this.value + '">' + (this.label || '') + '</span></div>';
                }
        ...
        var ui = new editorui.Combox({
            ...
            onselect:function (t, index) {
                editor.execCommand('forecolor', this.items[index].value);
            },
            onbuttonclick:function () {
                this.showPopup();
            }
        });
        editorui.buttons['forecolor'] = ui;
        editor.addListener('selectionchange', function (type, causeByUi, uiReady) {
            if (!uiReady) {
                var state = editor.queryCommandState('forecolor');
                if (state == -1) {
                    ui.setDisabled(true);
                } else {
                    ui.setDisabled(false);
