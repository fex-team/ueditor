...
(function () {
    ...
    function insertImage(imgObjs) {
        if( !editor.ui._dialogs.insertimagetypeDialog ) {
            editor.ui._dialogs.insertimagetypeDialog = new parentbaidu.editor.ui.insertimagetype( editor );
        }
        editor.ui._dialogs.insertimagetypeDialog.__img = (utils.isArray(imgObjs) ? imgObjs:[imgObjs]);
        editor.ui._dialogs.insertimagetypeDialog.onclick();
    ...
    function insertBatch() {
        ...
        for (var i = 0, ci; ci = imageUrls[i++];) {
            ...
            //增加后台传入的width和height
            ci.w && ( tmpObj.width = ci.w );
            ci.h && ( tmpObj.height = ci.h );
            imgObjs.push(tmpObj);
        }
        insertImage(imgObjs);
        hideFlash();
